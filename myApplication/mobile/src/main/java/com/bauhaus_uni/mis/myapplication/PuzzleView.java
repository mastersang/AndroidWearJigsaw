...
import com.bauhaus_uni.mis.myapplication.enums.Difficulty;
...
public class PuzzleView extends View implements OnGestureListener,
    ...
    private class MessageHandler extends Handler {
        public void handleMessage(Message message) {
            ...
            Double x = Math.toDegrees(Double.parseDouble(bundle.getString(gyX)));
            Double y = Double.parseDouble(bundle.getString(gyY));
            Double z = Double.parseDouble(bundle.getString(gyZ));
            Double gyroscopeRotationVelocity = Math.sqrt(x * x + y * y + z * z);
            ...
            if (tapped != null) {
                ...
                checkAngleAndPerformRotate();
            }
        }
    }

    private void checkAngleAndPerformRotate() {
        if (currentAngle >= 3 && previousAngle < 3) {
            tapped.turn(3);
            invalidate();
        } else if ((currentAngle <= -3 && previousAngle > -3) || (currentAngle <= -7 && previousAngle > -7)) {
            tapped.turn(1);
            invalidate();
        }
    }
